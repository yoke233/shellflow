...
export function ProjectTerminal({
  ...
  useEffect(() => {
    ...
    const initPty = async () => {
      ...
      // Spawn as 'project' type - this uses spawn_project_shell on the backend
      await spawnRef.current(projectId, 'project', cols, rows);

      // Mark as ready immediately for shell (no startup delay like main command)
      setIsReady(true);
  ...
  const handleRestart = useCallback(async () => {
    ...
    const rows = terminal.rows;
    spawnedAtRef.current = Date.now();
    await spawn(projectId, 'project', cols, rows);
    setIsReady(true);
  }, [spawn, projectId]);
